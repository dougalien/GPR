...
def disturbance_map(data: np.ndarray) -> np.ndarray:
    ...
    local_bg = uniform_filter(absd, size=(31, 41), mode="nearest")
    score = local_energy - local_bg
    score = gaussian_filter(score, sigma=(2.0, 2.0))
...
def hyperbola_score_map(data: np.ndarray, apex_half_width: int = 6) -> np.ndarray:
    ...
    smooth = gaussian_filter(data_abs, sigma=(1.2, 1.2))
    vertical_max = maximum_filter(smooth, size=(1, apex_half_width * 2 + 1))
    apex_mask = smooth >= vertical_max * 0.985
    ...
    score = 0.55 * smooth + 0.45 * curved_support
    score *= apex_mask.astype(float)
    score = gaussian_filter(score, sigma=(2.0, 1.0))
    score -= np.nanmin(score)
    if np.nanmax(score) > 0:
        score /= np.nanmax(score)
    return score
...
def candidate_table_from_scores(dist_score: np.ndarray, hyp_score: np.ndarray, time_axis: np.ndarray, distance_axis: np.ndarray, top_n: int = 12) -> pd.DataFrame:
    combo = 0.6 * dist_score + 0.4 * hyp_score
    local = maximum_filter(combo, size=(11, 21))
    peaks = np.argwhere((combo >= local) & (combo > np.quantile(combo, 0.985)))
    rows = []
    for tr, smp in peaks:
        rows.append({
            "Trace Index": int(tr),
            ...
            "Combined Score": float(combo[tr, smp]),
            "Likely Pattern": "possible hyperbolic target" if hyp_score[tr, smp] >= dist_score[tr, smp] else "possible disturbed zone",
        })
    if not rows:
        return pd.DataFrame(columns=["Trace Index", "Sample Index", "Distance", "Time", "Disturbance Score", "Hyperbola Score", "Combined Score", "Likely Pattern"])
    out = pd.DataFrame(rows).sort_values("Combined Score", ascending=False).head(top_n).reset_index(drop=True)
    return out
...
def process_gpr(gpr_data: GPRData, controls: Dict[str, Any]) -> ProcessedGPR:
    ...
    candidates = candidate_table_from_scores(dist_score, hyp_score, time_axis, distance_axis, top_n=int(controls["top_candidates"]))
    ...
    return ProcessedGPR(raw=raw, processed=proc, time_axis=time_axis, distance_axis=distance_axis, notes=notes, disturbance_score=dist_score, hyperbola_score=hyp_score, candidate_table=candidates)
